...
function create_service($data, $files = [])
{
    ...
    if ($service_id) {
        ...
        // Логирование
        $log_title = is_array($service_data['title']) ? json_encode($service_data['title'], JSON_UNESCAPED_UNICODE) : $service_data['title'];
        write_log("New service created: {$log_title} (ID: $service_id)", 'INFO');
        ...
        <div class="lg:hidden">
            ...
            <div class="flex flex-col gap-2">
                <a href="?action=create"
                    class="w-full inline-flex items-center justify-center px-4 py-3 bg-gradient-to-r from-primary-500 to-primary-600 text-white rounded-lg shadow-sm hover:from-primary-600 hover:to-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition duration-200 font-medium">
            ...
            <div class="p-4 border-b border-gray-200">
                <form method="GET" class="space-y-3">
                    ...
                    <button type="button" onclick="toggleMobileFilters()"
                        ...
                        <svg id="filter-arrow" class="w-4 h-4 ml-2 transition-transform duration-200" fill="none"
        ...
        <?php if (empty($services)): ?>
            <div class="text-center py-12">
                <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-4m-5 0H7m5 0v-5a2 2 0 00-2-2H8a2 2 0 00-2 2v5m10 0V9a2 2 0 00-2-2h-4a2 2 0 00-2 2v10">
                    </path>
                </svg>
                <h3 class="mt-2 text-sm font-medium text-gray-900">
                    <?php echo __('services.no_services', 'Услуги не найдены'); ?>
                </h3>
                <p class="mt-1 text-sm text-gray-500">
                    <?php echo __('services.no_services_description', 'Начните с создания первой услуги'); ?>
                </p>
                <div class="mt-6">
                    <?php render_button([
                        'href' => '?action=create',
                    ...
                    <div class="lg:hidden space-y-3">
                        <div class="flex items-center justify-between">
                            <label class="flex items-center">
                                ...
                                <span
                                    class="ml-2 text-sm font-medium text-gray-700"><?php echo __('common.select_all', 'Выбрать все'); ?></span>
                            </label>
                            <span id="selected-count-mobile" class="text-sm text-gray-500">0
                                <?php echo __('common.selected', 'выбрано'); ?></span>
                        </div>
                        <button type="submit" id="bulk-delete-btn-mobile"
                            class="w-full inline-flex items-center justify-center px-4 py-3 border border-transparent text-sm font-medium rounded-lg text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 disabled:opacity-50 disabled:cursor-not-allowed"
                            disabled>
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="hidden lg:flex items-center justify-between">
                        <div class="flex items-center space-x-4">
                            <label class="flex items-center">
                                ...
                                <span
                                    class="ml-2 text-sm text-gray-700"><?php echo __('common.select_all', 'Выбрать все'); ?></span>
                            </label>
                            <span id="selected-count-desktop" class="text-sm text-gray-500">0
                                <?php echo __('common.selected', 'выбрано'); ?></span>
                        </div>
                        <div class="flex items-center space-x-2">
                            <button type="submit" id="bulk-delete-btn-desktop"
                            ...
                            <div class="p-4">
                                <div class="flex items-start space-x-3">
                                    ...
                                    <div class="flex-shrink-0 pt-1">
                                        <input type="checkbox" name="selected_items[]" value="<?php echo $service['id']; ?>"
                                            class="item-checkbox-mobile w-4 h-4 rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                                    </div>
                                    ...
                                    <div class="flex-shrink-0">
                                        <?php if (!empty($service['image'])): ?>
                                            <img class="w-16 h-16 rounded-lg object-cover border border-gray-200"
                                                src="<?php echo htmlspecialchars($service['image']); ?>" alt="">
                                        <?php else: ?>
                                            <div
                                                class="w-16 h-16 rounded-lg bg-gray-100 border border-gray-200 flex items-center justify-center">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <div class="mt-4 flex flex-wrap gap-2">
                                            <a href="?action=edit&id=<?php echo $service['id']; ?>"
                                                class="flex-1 min-w-0 inline-flex items-center justify-center px-3 py-2 border border-gray-300 shadow-sm text-xs font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                                <?php echo get_icon('edit', 'w-4 h-4 mr-1'); ?>
                                                Редактировать
                                            </a>

                                            <button type="button" onclick="toggleServiceStatus(<?php echo $service['id']; ?>)"
                                                class="flex-1 min-w-0 inline-flex items-center justify-center px-3 py-2 border border-gray-300 shadow-sm text-xs font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                                <?php echo $service['status'] === 'active' ? __('services.deactivate', 'Скрыть') : __('services.activate', 'Показать'); ?>
                                            </button>

                                            <button type="button"
                                                onclick="confirmDeleteService(<?php echo $service['id']; ?>, '<?php echo htmlspecialchars($service['title']); ?>')"
                                                class="flex-1 min-w-0 inline-flex items-center justify-center px-3 py-2 border border-transparent text-xs font-medium rounded-lg text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                    ...
                    <table class="min-w-full divide-y divide-gray-200 admin-table-responsive">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th
                                    class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[120px]">
                                    <?php echo __('services.category', 'Категория'); ?>
                                </th>
                                <th
                                    class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[100px]">
                                ...
                                <th
                                    class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[80px]">
                                    <?php echo __('services.priority', 'Приоритет'); ?>
                                </th>
                                <th
                                    class="px-3 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[300px]">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <?php foreach ($services as $service): ?>
                                <tr class="hover:bg-gray-50">
                                    <td class="px-3 py-4">
                                        <input type="checkbox" name="selected_items[]" value="<?php echo $service['id']; ?>"
                                            class="item-checkbox-desktop rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                                    </td>
                                    <td class="px-3 py-4">
                                        <div class="flex items-center">
                                            <?php if (!empty($service['image'])): ?>
                                                ...
                                                <?php else: ?>
                                                    ...
                                                    <div class="text-sm text-gray-500">
                                                        ...
                                                        <?php echo strlen($service['description']) > 60 ? '...' : ''; ?>
                                                    </div>
                                                </div>
                                            </div>
                                    </td>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <?php if (!empty($current_service['image'])): ?>
                                <div class="mb-4">
                                    <p class="text-sm text-gray-600 mb-2">
                                        <?php echo __('common.current_image', 'Текущее изображение'); ?>:
                                    </p>
                            ...
                            <div id="image-preview" class="hidden mt-4">
                                <p class="text-sm text-gray-600 mb-2">
                                    <?php echo __('common.new_image_preview', 'Превью нового изображения'); ?>:
                                </p>
