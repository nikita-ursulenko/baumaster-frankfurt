...
// Простая защита - разрешаем доступ с локального хоста или если мы в режиме отладки
if (!in_array($_SERVER['REMOTE_ADDR'] ?? '', ['127.0.0.1', '::1']) && !is_debug()) {
    die('Доступ запрещен');
}
...
try {
    ...
    if ($result) {
        ...
        echo "<p><strong>URL админки:</strong> <a href='{$login_url}'>{$login_url}</a></p>";
